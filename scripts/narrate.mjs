...
You are given facts about a set of games and a set of team storylines that have already been
selected by a separate process -- your only job is to phrase them, not to judge which ones matter.
Use ONLY the facts provided. Do not invent stats, injuries, records, or history not given to you.
Each blurb should be 1-2 sentences, written for a knowledgeable college football fan, no hashtags
or emoji. Return your answer via the write_blurbs tool.`;
...
async function fetchBlurbs(facts) {
  ...
  const response = await client.messages.create({
    ...
    messages: [
      {
        role: 'user',
        content: `Write blurbs for these games and predictions:\n\n${JSON.stringify(facts, null, 2)}`,
      },
    ],
  });
...
async function main() {
  ...
  let result = null;
  if (!facts.games.length && !facts.predictions.length) {
    console.log('No games or predictions to narrate.');
  } else {
    try {
